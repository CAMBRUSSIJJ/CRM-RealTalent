...
const file = path.join(backupDir, `realtalent-${version}-${timestamp}.dump`)
...
const manifest = { application:'RealTalent CRM', version, createdAt:new Date().toISOString(), file:path.basename(file), bytes:fs.statSync(file).size, sha256, databaseHost:parsed.hostname }
