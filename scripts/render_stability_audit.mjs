...
check('Gravação recolhível', callModal.includes('call-recording-tools') && callModal.includes('<summary><Mic'), 'controles exibidos sob demanda')
...
const report = { version: '100.46.4', generatedAt: new Date().toISOString(), passed: checks.every((item) => item.passed), checks }
fs.writeFileSync(path.join(root, 'RENDER-STABILITY-AUDIT-V100-46-4.json'), JSON.stringify(report, null, 2) + '\n')
