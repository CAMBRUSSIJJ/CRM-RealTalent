...
def onboard(page):
 ...
 page.get_by_label('Seu nome').fill('Homologação V100.46.4'); page.get_by_label('Nome da empresa').fill('RealTalent Homologação')
 page.get_by_role('button',name='Entrar no CRM').evaluate('el => el.click()'); page.get_by_role('heading',name='Meu Dia').wait_for(); page.wait_for_timeout(700)
...
try:
 with sync_playwright() as p:
  ...
  assert page.locator('.sidebar__item',has_text='Comunicações').count()==0; results['checks'].append('Comunicações removida da navegação')
  for label_text,heading in [('Leads','Leads'),('Mapa de Leads','Mapa de Leads'),('Ligações','Ligações'),('Configurações','Configurações')]:
   page.locator('.sidebar__item',has_text=label_text).click(); page.get_by_role('heading',name=heading).wait_for(); no_overflow(page,label_text)
  page.locator('.sidebar__item',has_text='Leads').click(); page.get_by_role('heading',name='Leads').wait_for(); page.wait_for_timeout(300)
  assert page.get_by_text('O CRM encontrou um problema inesperado').count()==0; results['checks'].append('Leads renderiza sem ErrorBoundary')
  page.locator('.sidebar__item',has_text='Ligações').click(); page.get_by_role('heading',name='Ligações').wait_for()
  page.get_by_role('button',name='Começar rotina').click(); page.get_by_role('heading',name='Modo Ligação em Foco').wait_for(); no_overflow(page,'Modo Ligação')
  assert page.get_by_role('button',name='Objeções').count()>=1; assert page.get_by_role('button',name='Notas').count()>=1
  results['checks'].append('Modo Ligação simplificado abre com apoio em abas')
