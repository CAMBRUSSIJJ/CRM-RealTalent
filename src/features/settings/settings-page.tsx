...
import { useExperience, type ExperiencePreset } from '../experience/experience-context'
...
type SettingsTab = 'account' | 'experience' | 'company' | 'navigation' | 'team' | 'commercial' | 'integrations' | 'data'
...
export function SettingsPage() {
  ...
  const tabItems: Array<{ id: SettingsTab; label: string; icon: typeof UserCog }> = [
    { id: 'account', label: 'Minha conta', icon: UserCog }, { id: 'experience', label: 'Experiência', icon: Palette }, { id: 'company', label: 'Empresa', icon: Building2 },
    ...
    <div className="settings-v16__body">
      ...
      <main className="settings-v16__content page-stack">
        ...
        {activeTab === 'experience' ? <>
          <article className="panel experience-settings-card"><div className="panel__heading"><div><span className="eyebrow">Experiência individual</span><h3>Perfil de trabalho</h3><p>Aplique uma base profissional e refine cada página pelo botão Personalizar visualização.</p></div><Palette size={22} /></div><div className="settings-choice-grid experience-settings-presets">{([['sdr', 'SDR', 'Prospecção e volume'], ['seller', 'Vendedor', 'Carteira e execução'], ['closer', 'Closer', 'Propostas e fechamento'], ['manager', 'Gerente', 'Equipe e forecast'], ['executive', 'Executivo', 'Indicadores e decisões'], ['admin', 'Administrador', 'Configuração completa']] as Array<[ExperiencePreset, string, string]>).map(([id, label, detail]) => <label key={id} className={`settings-choice ${experience.preferences.preset === id ? 'is-active' : ''}`}><input type="radio" name="experience-preset" checked={experience.preferences.preset === id} onChange={() => experience.setPreset(id)} /><strong>{label}</strong><span>{detail}</span></label>)}</div></article>
          <article className="panel"><div className="panel__heading"><div><span className="eyebrow">Leitura e acessibilidade</span><h3>Conforto visual global</h3></div><LayoutGrid size={22} /></div><div className="settings-form-grid settings-form-grid--3"><label className="field"><span>Largura do conteúdo</span><select value={experience.preferences.global.contentWidth} onChange={(event) => experience.updateGlobal({ contentWidth: event.target.value as typeof experience.preferences.global.contentWidth })}><option value="focused">Focada</option><option value="wide">Ampla</option><option value="fluid">Fluida</option></select></label><label className="field"><span>Tamanho da fonte</span><select value={experience.preferences.global.fontScale} onChange={(event) => experience.updateGlobal({ fontScale: event.target.value as typeof experience.preferences.global.fontScale })}><option value="small">Menor</option><option value="medium">Padrão</option><option value="large">Maior</option></select></label><label className="field"><span>Contraste</span><select value={experience.preferences.global.contrast} onChange={(event) => experience.updateGlobal({ contrast: event.target.value as typeof experience.preferences.global.contrast })}><option value="standard">Padrão</option><option value="high">Reforçado</option></select></label><label className="settings-toggle"><input type="checkbox" checked={experience.preferences.global.stickyTopbar} onChange={(event) => experience.updateGlobal({ stickyTopbar: event.target.checked })} /><span><strong>Barra superior fixa</strong><small>Mantém busca e ações disponíveis.</small></span></label><label className="settings-toggle"><input type="checkbox" checked={experience.preferences.global.showPageSubtitle} onChange={(event) => experience.updateGlobal({ showPageSubtitle: event.target.checked })} /><span><strong>Subtítulos das páginas</strong><small>Exibe o contexto da área atual.</small></span></label><label className="settings-toggle"><input type="checkbox" checked={experience.preferences.global.reduceTransparency} onChange={(event) => experience.updateGlobal({ reduceTransparency: event.target.checked })} /><span><strong>Reduzir transparência</strong><small>Melhora a legibilidade de superfícies.</small></span></label></div></article>
          <article className="panel"><div className="panel__heading"><div><span className="eyebrow">Central de notificações</span><h3>Avisos que devem aparecer</h3></div><Gauge size={22} /></div><div className="settings-form-grid settings-form-grid--2"><label className="settings-toggle"><input type="checkbox" checked={experience.preferences.notifications.overdueActivities} onChange={(event) => experience.updateNotifications({ overdueActivities: event.target.checked })} /><span><strong>Atividades vencidas</strong><small>Follow-ups que já passaram do prazo.</small></span></label><label className="settings-toggle"><input type="checkbox" checked={experience.preferences.notifications.upcomingEvents} onChange={(event) => experience.updateNotifications({ upcomingEvents: event.target.checked })} /><span><strong>Próximos compromissos</strong><small>Eventos nas próximas 24 horas.</small></span></label><label className="settings-toggle"><input type="checkbox" checked={experience.preferences.notifications.proposalFollowups} onChange={(event) => experience.updateNotifications({ proposalFollowups: event.target.checked })} /><span><strong>Propostas para acompanhar</strong><small>Propostas próximas da data esperada.</small></span></label><label className="settings-toggle"><input type="checkbox" checked={experience.preferences.notifications.automationFailures} onChange={(event) => experience.updateNotifications({ automationFailures: event.target.checked })} /><span><strong>Falhas de automação</strong><small>Execuções que precisam de revisão.</small></span></label></div><div className="settings-actions"><Button variant="secondary" onClick={experience.resetAll}><RotateCcw size={16} /> Restaurar experiência individual</Button><span className="settings-copy">{experience.syncing ? 'Sincronizando preferências...' : experience.lastSyncedAt ? 'Preferências sincronizadas com seu usuário.' : 'Preferências salvas neste dispositivo.'}</span></div></article>
        </> : null}
