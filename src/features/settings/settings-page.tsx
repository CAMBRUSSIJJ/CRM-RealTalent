...
function StageRow({ stage }: { stage: PipelineStage }) {
  ...
  return <div className="settings-stage-row">
    ...
    <button className="icon-button" type="button" aria-label={`Excluir ${stage.name}`} onClick={() => { if (confirm(`Excluir a etapa ${stage.name}? Ela precisa estar sem leads.`)) void app.deleteStage(stage.id) }}><Trash2 size={16} /></button>
  </div>
}
    ...
    <div className="settings-v16__body">
      ...
      <main className="settings-v16__content page-stack">
        ...
        {activeTab === 'data' ? <>
          ...
          {canManageTeam ? <article className="panel danger-zone"><div className="panel__heading"><div><span className="eyebrow">Zona de risco</span><h3>Limpeza e restauração visual</h3></div><Trash2 size={22} /></div><div className="danger-zone__grid"><div><strong>Restaurar configurações</strong><p>Volta tema, marca, navegação e regras comerciais ao padrão do workspace. Os leads não são alterados.</p><Button variant="secondary" onClick={() => { if (confirm('Restaurar todas as configurações visuais e comerciais?')) { resetPreferences(); notify('success', 'Configurações restauradas.') } }}><RotateCcw size={16} /> Restaurar padrões</Button></div><div><strong>Limpar dados operacionais</strong><p>Remove leads, atividades, ligações, agenda, playbooks, metas e automações. Mantém o workspace, equipe, Pipeline e configurações.</p><label className="field"><span>Digite APAGAR para confirmar</span><input value={dangerText} onChange={(event) => setDangerText(event.target.value)} /></label><Button variant="danger" loading={clearing} disabled={dangerText !== 'APAGAR'} onClick={() => void clearWorkspace()}><Trash2 size={16} /> Limpar dados</Button></div></div></article> : null}
        </> : null}
